...
class _DoctorAppointmentListScreenState
  ...
  Widget build(BuildContext context) {
    final appointmentProvider =
        Provider.of<HospitalAppointmentService>(context);
        ...
        children: [
          ...
          if (_selectedFilterIndex ==
              filterOptions.length -
                  1) // Step 4: Show the date picker if "By date" is selected
            DateSelectionCard(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  final filteredAppointments =
                      filterAppointments(snapshot.data!);
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      initialDate: _selectedDate ??
          DateTime
              .now(), // Use selected date if available, else use today's date
  ...
  List<Appointment> filterAppointments(List<Appointment> appointments) {
    switch (_selectedFilterIndex) {
      ...
      case 1:
        return appointments
            .where((appointment) => isToday(appointment.appointmentDate))
            .toList();
      case 2:
        return appointments
            .where((appointment) => isTomorrow(appointment.appointmentDate))
            .toList();
      // case 3:
      ...
      case 3: // Filter by selected date
        return appointments
            .where((appointment) =>
                isSameDate(appointment.appointmentDate, _selectedDate!))
            .toList();
    }
    return appointments;
  }
